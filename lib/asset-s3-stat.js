...
s3stat.Task = function( fileObj, options, target, s3ops ) {
  ...
  this.options.access = this.options.access || 'private';

  this.options.sslEnabled = true;
  this.options.logger = process.stdout;

  AWS.config.update({accessKeyId: this.options.key, secretAccessKey: this.options.secret});
...
s3stat.Task.prototype._startUpload = function( assetObj ) {
  ...
  fs.readFile(assetObj.absPath, function (err, data) {
    if (err) {
    } else {
      var type = mime.contentType(mime.lookup(assetObj.absPath) || 'application/octet-stream');
      var params = {
        ACL        : options.access,
        Body       : data,
      ...
      s3.putObject(params, function(err) {
        if (err) {
          log.error(
            'Failed to upload: ' + assetObj.file.blue + ' to: ' +
...
s3stat.Task.prototype._s3Head = function( assetObj ) {
  ...
  this.s3.headObject(params, function(err, data){
    ...
    log.debug( this.debug, 's3stat.Task._s3Head() :: AWS Response: ' +
      'err,count,asset: ' + (err+'').red + ', ' +
      (++this._headCount + '').cyan +
      ', ' + s3assetPath.blue);

    if (err) {
      log.error('S3 HEAD response error: ' + s3assetPath.blue + ' ' + err );
 ...
 */
s3stat.Task.prototype._getConfig = function(options) {
  var defaultOpts = {
  ...
  // Combine the options and defaultOptions as the config
